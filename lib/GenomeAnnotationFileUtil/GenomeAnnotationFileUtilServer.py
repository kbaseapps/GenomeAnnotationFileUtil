...
import os
from GenomeAnnotationFileUtil.authclient import KBaseAuth as _KBaseAuth
...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'GenomeAnnotationFileUtil'):
        retconfig[nameval[0]] = nameval[1]
    return retconfig

config = get_config()

from GenomeAnnotationFileUtil.GenomeAnnotationFileUtilImpl import GenomeAnnotationFileUtil  # @IgnorePep8
impl_GenomeAnnotationFileUtil = GenomeAnnotationFileUtil(config)
                ...
                try:
                    ...
                    if auth_req != 'none':
                        if token is None and auth_req == 'required':
                            err = JSONServerError()
                            err.data = (
                                'Authentication required for GenomeAnnotationFileUtil ' +
